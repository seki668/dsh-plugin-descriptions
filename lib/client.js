window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region host metadata (自动读取任意已安装插件的 package.json 描述)
		// 宿主侧 /api/dsh-plugin-descriptions 在请求时遍历 Loader 条目并读取
		// 每个插件包自己的 package.json 描述。这里缓存结果；失败时下次重试。
		let HOST_DESCRIPTIONS = {};
		let hostMetaLoad = null;
		function loadHostDescriptions() {
			if (hostMetaLoad === null) {
				hostMetaLoad = fetch("/api/dsh-plugin-descriptions", { headers: { accept: "application/json" } })
					.then((response) => response.ok ? response.json() : { descriptions: {} })
					.then((payload) => {
						HOST_DESCRIPTIONS = payload !== null && typeof payload === "object" && payload.descriptions !== null && typeof payload.descriptions === "object"
							? payload.descriptions
							: {};
						return HOST_DESCRIPTIONS;
					})
					.catch(() => {
						hostMetaLoad = null;
						return {};
		...
		function hostDescriptionOf(entry) {
			const meta = HOST_DESCRIPTIONS[entry.moduleName];
			return meta !== null && typeof meta === "object" && typeof meta.description === "string" && meta.description.length > 0
				? meta.description
				: "";
		}
		function sourceOf(entry) {
			if (ENTRY_NOTES[entry.entryId] !== void 0 || DESCRIPTIONS[entry.moduleName] !== void 0) return "manual";
			if (hostDescriptionOf(entry) !== "") return "auto";
			return "none";
		}
		//#endregion
		...
		function descriptionOf(entry, t) {
			...
			const hostDescription = hostDescriptionOf(entry);
		...
		const styles = {
			...
			sourceTag: { fontSize: 10, lineHeight: "14px", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 4, padding: "0 4px", color: "var(--dsw-alias-label-tertiary)", whiteSpace: "nowrap" },
			footer: { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, fontSize: 11, lineHeight: "16px", color: "var(--dsw-alias-label-tertiary)" }
		};
		...
		function renderEntryCard(entry, tt) {
			...
			return react.createElement("li", { key: entry.entryId, style: styles.card, "data-plugin-entry": entry.entryId },
				...
				react.createElement("div", { style: styles.footer },
					...
					source !== "none" ? react.createElement("span", { style: styles.sourceTag }, source === "auto" ? tt("sourceAuto") : tt("sourceManual")) : null,
		...
		function PluginGuideTab({ list, t }) {
			...
			const [, setHostMeta] = react.useState(0);
			react.useEffect(() => {
				let current = true;
				Promise.all([loadHostDescriptions(), list()]).then(([, snapshot]) => {
					if (!current) return;
					setHostMeta((value) => value + 1);
		...
		exports.__debug = { shortName, descriptionOf, matches, categoryOf, renderEntryCard, loadHostDescriptions, hostDescriptionOf, sourceOf, DESCRIPTIONS, ENTRY_NOTES, CATEGORY_ORDER, CATEGORY_OVERRIDES };
